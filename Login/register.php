...
if ($ok)
{
    ...
    $stmt = $conn->prepare($sql);
    $stmt->execute([':id' => NULL, ':dni' => $dni, ':name' => $user, ':surname' => $surname1, ':surname2' => $surname2, ':phone' => $phone, ':email' => $email, ':pass' => $encrypted, ':bday' => $bday, ':path' => $path, ':hash' => $hash, ':active' => 0]);
    ...
    echo "<script>if (!alert('Ya estás dado de alta, consulta tu E-mail para confirmar tu inscripción.')) window.open('index.php', '_self');</script>";
